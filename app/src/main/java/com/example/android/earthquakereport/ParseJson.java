...
//
    ...
    public static ArrayList<Earthquake> extractEarthquakes(String queryData) {
        ...
        try {
           ...
           // SimpleDateFormat dateFormat = new SimpleDateFormat("MMM DD,yyyy h:mm a", Locale.ENGLISH);
            ...
            for (int i = 0; i < features.length(); i++)
            {
                ...
                String formattedString = magnitudeFormat.format(mag);
                String place = properties.getString("place");
                String[] parts = place.split(" of ");
                long timeValue = properties.getLong("time");
                String date = dateFormat.format(timeValue);
                String time = timeFormat.format(timeValue);
                String url = properties.getString("url");
                if (parts.length<=1) {
                    earthquakes.add(new Earthquake(formattedString, "Near to", parts[0], date, time, url));
                }
                else {
                    earthquakes.add(new Earthquake(formattedString, parts[0], parts[1], date, time, url));
                }
            }

        } catch (JSONException e) {
            Log.e("ParseJson", "Problem parsing the earthquake JSON results", e);
        }
